...
<?php 
    ...
    while($row = mysqli_fetch_array($result)){
        ...
        if($no == 0){
          ...
          if($row['room_quantity'] > $row['room_occupied']){
            $res = mysqli_query($conn , "update rooms set room_status = 'available' where room_id = $row[room_id] ");
          }else{
            $res = mysqli_query($conn , "update rooms set room_status = 'unavailable' where room_id = $row[room_id]");
          }
    ...
    foreach($_SESSION['rooms'] as $key => $room){
        ...
        if($room['room_quantity'] > $room['room_occupied']){
          ...
          <form action = 'reservationTransaction.php?action=add&id=$room[id]' method = 'POST'>
