...
//WHERE checkout =3";//*where id = '$pid'*/ "
 $userst = "<a href='users.php' class='list-group-item'><span class='' aria-hidden='true'></span> Users <span class='badge'>";
                ...
                $sql2 = "SELECT * FROM users where user_id =34";
...
//hindi ko alam kung para saan itomg fucntion pero kapag dinelete mo siya wala namang mangyayari sa system
    ...
    $this->SetFont('','B', 10);
		...
		$this->Cell(48,15,'Customers Name',1,0,'C',true);
		$this->Cell(27,15,'Checkin',1,0,'C',true);
		$this->Cell(27,15,'Checkout',1,0,'C',true);
		$this->Cell(30,15,'Total Amount',1,0,'C',true);//C for Center 'L' for Left 'R' for right
		$this->Cell(30,15,'Amount Paid',1,0,'C',true);
		$this->Cell(22,15,'Status',1,0,'C',true);
 ...
 while($row=mysqli_fetch_array($re))
  {
   ...
   //$allTotal = $allTotal+($roomRate *$checkout);
    ...
    $nodays = $row['nodays'];*/
   // if($checkin >= $checkin1){
    if($checkout1 >= $checkin2 && $checkout2 >=$checkout1 ){
    	 	$n++;
    		$pdf->Cell(10,10,''.$n,1,0,'L');
			$pdf->Cell(48,10,''.$cust_name,1,0,'L');
			$pdf->Cell(27,10,''.$checkin,1,0,'L');
			$pdf->Cell(27,10,''.$checkout,1,0,'L');
			$pdf->Cell(30,10,'P'.$total_amount.'.00',1,0,'L');
			$pdf->Cell(30,10,'P'.$amount_paid.'.00',1,0,'L');
			//$pdf->Cell(10,10,''.$cash_rendered.'.00',1,0,'L');
			$pdf->Cell(22,10,''.$status,1,0,'L');
